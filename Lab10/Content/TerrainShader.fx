...
struct VertexInput
{
	...
	//float4 Normal : NORMAL0; // are called mnemonics      //we're not using it
};
...
{
	float3 normal = tex2D(NormalSampler, input.UV).xzy; // Unsureeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
	...
	float3 diffuse = max(dot(lightDirection, normal), 0) * tex2D(NormalSampler, input.UV).xyz;
