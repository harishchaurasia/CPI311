...
namespace CPI311.GameEngine
{
    public class TerrainRenderer : Component
    {
        ...
        public float GetHeight(Vector2 tex) // tx is UV [0,1]
        {
            // First, scale it to dimensions of the image
            tex = Vector2.Clamp(tex, Vector2.Zero, Vector2.One) *
                        new Vector2(HeightMap.Width - 1, HeightMap.Height - 1); int x = (int)tex.X; float u = tex.X - x;
        ...
        public float GetAltitude(Vector3 position)
        {
            ...
            if (position.X > -size.X / 2 && position.X < size.X / 2 &&
                ...
                return GetHeight(
                    new Vector2((position.X + size.X / 2) / size.X, (position.Z + size.Y / 2) / size.Y)) * Transform.LocalScale.Y;
            return -1;
        }
        ...
        public void Draw()
        {
                ...
                ScreenManager.GraphicsDevice.DrawUserIndexedPrimitives<VertexPositionTexture>(PrimitiveType.TriangleList, Vertices, 0, Vertices.Length, Indices, 0, Indices.Length / 3);
        }
    }
}